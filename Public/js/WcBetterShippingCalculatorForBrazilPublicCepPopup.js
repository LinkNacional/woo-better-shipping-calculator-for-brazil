...
(function ($) {
    ...
    function renderBackResult($back, success, data) {
        ...
        if (success) {
            ...
            var raw = (getAjaxData().whatsapp_number || '').replace(/\D/g, '');
            var whatsapp = raw;
            // Formata: se o usuário já incluiu o código do país (55), usa direto;
            // caso contrário, prefixa com 55.
            if (whatsapp) {
                if (whatsapp.indexOf('55') !== 0) {
            ...
            html += '<p class="wc-better-cep-back-msg">' + escapeHtml(data.message || (getAjaxData().fail_message || 'Entre em contato conosco para organizar sua entrega.')) + '</p>';
            html += '<div class="wc-better-cep-popup-back-actions">';
            html += '<button class="wc-better-cep-popup-back-btn">Voltar</button>';
            if (whatsapp) {
                html += '<a class="wc-better-cep-popup-whatsapp-btn" href="https://wa.me/' + whatsapp + '?text=' + encodeURIComponent(waMsg) + '" target="_blank" rel="noopener">';
